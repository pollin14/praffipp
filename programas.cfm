...
<table width="950" border="0" align="center" cellpadding="0" cellspacing="2">
  ...
  <tr>
    <td width="600" rowspan="2" align="center" class="colfooter">2012</td>
    <td colspan="5" align="center" class="colfooter">Recursos</td>
  </tr>
  <tr class="colfooter">
    <td width="80" align="center">Recurso federal asignado para el a&ntilde;o 2012</td>
    <td width="80" align="center">Total  de Egresos</td>
    <td width="80" align="center">%</td>
    <td width="80" align="center">Disponibilidad Financiera</td>
    <td width="80" align="center">%</td>
  </tr>
 ...
 <cfquery name="Nom_Prog" dbtype="query"><!--- Solo los principales y únicos (descartamos secundarios)--->
 	select Id_MainProg,Id_Prog,Nom_Prog from programas where Jerarquia<>'s' group by Id_MainProg,Id_Prog,Nom_Prog
 </cfquery>
 <cfquery name="montos" datasource="prafipp"> <!--- eliminamos los respaldos --->
 	select * from montos where obsoleto=0
 </cfquery>
 <cfset Id_Progs=#SESSION.Id_Prog#>
...
<cfif #find(",",SESSION.Id_Prog)# GT 0>
		<cfoutput>
          <cfloop query="Nom_Prog">
            ...
            <cfset Num_Egresos = Q_Jerarquia.Num_Egresos >
            <cfset Jerarquia = Q_Jerarquia.Jerarquia>
                <!---cfoutput> EGR=#Num_Egresos# </cfoutput--->
                <cfquery name="Egresos" dbtype="query">
                    select sum (monto1) as Monto1, sum (monto2) as Monto2, sum (monto3) as Monto3, sum (monto4) as Monto4 from montos where Id_Rubro > 4 and Id_Rubro <= #Num_Egresos#+#SESSION.num_Ingresos# and Id_Prog=#Nom_Prog.Id_Prog#
                </cfquery>        
                <td>
              <cfif #Jerarquia# eq "u">
              <!---Preguntamos por los programas a los q tiene acceso --->
              <cfif #find(Nom_Prog.Id_Prog,Id_Progs)# GT 0>
                    <cfif #SESSION.rol# GT 6>
                        <a href="itaf_cap.cfm?Id_Prog=#Nom_Prog.Id_Prog#&Id_EntFed=#SESSION.Id_EntFed#">#Nom_Prog.Nom_Prog#</a>
                    <cfelse>
                        <a href="colorama.cfm?Id_Prog=#Nom_Prog.Id_Prog#" title="Clic aquí para ver desglose nacional">#Nom_Prog.Nom_Prog#</a>
                    </cfif>
              <cfelse>
                    <cfoutput>#Nom_Prog.Nom_Prog#</cfoutput>
              </cfif>
                    <cfset QTotal_Egresos = "select sum(monto1+monto2+monto3+monto4) as Total_Egresos from montos where Id_Prog=#Nom_Prog.Id_Prog# and obsoleto=0 and Id_Rubro > 4 and Id_Rubro <= #Num_Egresos#+#SESSION.num_Ingresos# and 2=2">
                <!---cfoutput> #QTotal_Egresos # </cfoutput--->
                <!---cfoutput> EGR=#Num_Egresos# </cfoutput--->
                  <cfelse>
                    #Nom_Prog.Nom_Prog#
                    <cfquery name="Lista_Hijos" dbtype="query">
                        select Id_Prog from programas where Id_MainProg=#Nom_Prog.Id_Prog# and jerarquia='s'
                    </cfquery>
                    <cfset Hijos_str="">
                    ...
                    <!---cfset QTotal_Egresos = "select sum(monto1+monto2+monto3+monto4) as Total_Egresos from montos where Id_Prog in #Hijos_str# and obsoleto=0 and Id_Rubro > 4 and Id_Rubro <= #Num_Egresos#+#SESSION.num_Ingresos# and 3=3"--->
                    <cfset QTotal_Egresos = "select sum(monto1+monto2+monto3+monto4) as Total_Egresos from montos where Id_Prog in #Hijos_str# and obsoleto=0 and Id_Rubro > 4">
                <!---cfoutput> #QTotal_Egresos # </cfoutput--->
                  </cfif>
                </td>
                ...
                <cfloop query="hijos">
                    ...
                    <!---Preguntamos por los programas a los q tiene acceso --->
                    <cfif #find(Hijos.Id_Prog,Id_Progs)# GT 0>
                        <cfif #SESSION.rol# GT 6>
                            <td><a href="itaf_cap.cfm?Id_Prog=#Hijos.Id_Prog#&Id_EntFed=#SESSION.Id_EntFed#"> --#hijos.Nom_Prog#</a></td>
                        <cfelse>            
                            <td><a href="colorama.cfm?Id_Prog=#Hijos.Id_Prog#" title="Clic aquí para ver desglose nacional"> --#hijos.Nom_Prog#</a></td>
                        </cfif>
                    <cfelse>
                        <td><cfoutput>--#hijos.Nom_Prog#</cfoutput></td>
                    </cfif>
                    <td  align="right">#NumberFormat( hijos.monto, "," )#</td>
                    <!---cfset total+=hijos.monto--->
                    <td  align="right">
        
                    <cfset QTotal_Egresos = "select sum(monto1+monto2+monto3+monto4) as Total_Egresos from montos where Id_Prog=#hijos.Id_Prog# and obsoleto=0 and Id_Rubro > 4 and Id_Rubro <= #Num_Egresos#+#SESSION.num_Ingresos# and 1=1">
                <!---cfoutput > #QTotal_Egresos# </cfoutput --->
